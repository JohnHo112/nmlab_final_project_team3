...
function saveToFile(filename:any, jwk:any, keyId:any) {
    const dataToSave = {
        jwk: jwk,
        keyId: keyId,
    };
    writeFileSync(filename, JSON.stringify(dataToSave, null, 2));
    console.log("write succ");
  }

function loadFromFile(filename:any) {
...
export async function createVC() {
    ...
    let jwk = new JwkMemStore();
    let keyId = new KeyIdMemStore();

    // Create an identity for the issuer with one verification method `key-1`.

    let issuerStorage: Storage = new Storage(jwk, keyId);
    saveToFile("hello.json", jwk, keyId);
    ...
    console.log("issuerDocument.id");
    console.log(issuerDocument);
